...
**3. Access Control**
We are going to porcseing data from people's houses, so ew have to maintain a straict control of data access. Instead of giving everyone the logh gcreatial to aws for image uploading, the right of processing data internally will be given to certain persons.Pckyuderm will 
Pracicy extra layer basic facility. Trakc user id performing tracking. Id are unique to object stroagre, not necessrtiey unique to a certain person. Howve to make that connection. Basic quthentica system. Processing data form hurned thousand differnet house a lot management overhead creatae a huednr thaousna d diferetn buckts. Manually mange the acesss to all of those, ideally we  want to just throuw data to s3 and mange who have the access.to which data interally.
